...
export default function Sidebar() {
  ...
  const handleLogout = () => {
    localStorage.removeItem('chamber_power_hub_auth');
    router.push('/power-hub');
  };

  return (
    <aside className="w-64 bg-[#4B2E83] text-white min-h-screen flex flex-col">
      {/* Logo */}
      <div className="p-6 border-b border-white/10">
        <Link href="/power-hub/dashboard" className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-white/60">Murray Chamber</p>
          </div>
        </Link>
      </div>
