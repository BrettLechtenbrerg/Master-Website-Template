      ...
      <div className="p-8">
        ...
        <div className="flex flex-wrap items-center justify-between gap-4 mb-6">
          <div className="flex items-center gap-4">
            <button
              ...
              disabled={uploading}
              className="flex items-center gap-2 px-4 py-2 bg-[#4B2E83] text-white rounded-lg hover:bg-[#7A59B5] transition-colors disabled:opacity-50"
            >
        ...
        <div
          ...
          className={`border-2 border-dashed rounded-xl p-8 mb-6 text-center transition-all cursor-pointer ${
            isDragging ? 'border-[#4B2E83] bg-[#4B2E83]/5' : 'border-gray-300 hover:border-[#4B2E83]'
          }`}
        >
          <Upload size={40} className={`mx-auto mb-4 ${isDragging ? 'text-[#4B2E83]' : 'text-gray-400'}`} />
              ...
              <div key={file.id} className="bg-white rounded-xl border border-gray-200 p-3 group">
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex gap-2">
                    <button
                      onClick={() => copyUrl(file.url)}
                      className="flex-1 flex items-center justify-center gap-1.5 py-2 bg-[#4B2E83] text-white text-sm rounded-lg hover:bg-[#7A59B5] transition-colors"
                    >
            ...
            {filteredMedia.map((file) => (
              <div key={file.id} className="flex items-center justify-between p-4 border-b border-gray-100 last:border-0 gap-4">
                ...
                <div className="flex items-center gap-2 flex-shrink-0">
                  ...
                  <button
                    onClick={() => copyUrl(file.url)}
                    className="flex items-center gap-1.5 px-3 py-2 bg-[#4B2E83] text-white text-sm rounded-lg hover:bg-[#7A59B5] transition-colors"
                  >
                    {copiedUrl === file.url ? <Check size={14} /> : <Copy size={14} />}
                    {copiedUrl === file.url ? 'Copied!' : 'Copy URL'}
                  </button>
